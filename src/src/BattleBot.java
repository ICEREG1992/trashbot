...
public class BattleBot {
    ...
    public void battle(MessageCreateEvent event) {
        ...
        else if (messageToString.contains("!attack") && message.getAuthor().getId()!=selfID) {
            attack(health, botHealth, choice, userEmoji, healthBar, battleMessage);
            if (choice == 0) {
                channel.sendMessage("A battle is not going on right now. Type ``!battle`` to start one!");
            }
        }

        else if (messageToString.contains("!heal") && message.getAuthor().getId()!=selfID) {
            heal(health, botHealth, choice, userEmoji, healthBar, battleMessage);
            if (choice == 0) {
                channel.sendMessage("A battle is not going on right now. Type ``!battle`` to start one!");
            }
        }

        else if (messageToString.contains("!run") && message.getAuthor().getId()!=selfID) {
            run(health, botHealth, choice, userEmoji, healthBar, battleMessage);
            if (choice == 0) {
                channel.sendMessage("A battle is not going on right now. Type ``!battle`` to start one!");
            }
        }
